...
class ForkedPathsNode extends ForkedPathsBase
{
     ...
     * @var string
     */
    public const MAGIC_KEYWORD = '##FORKEDPATHS##';
     ...
     * @var bool
     */
    public static $REMOVE_CHILDREN_FROM_INDEX = true;
     ...
     * @var int
     */
    public const MAX_BUTTONS_PER_ROW = 3;
     ...
     *
     * @param \Lynxlab\ADA\Main\Node\Node $node
     * @return bool
     */
    ...
    public static function removeForkedPathsChildrenFromIndex($courseData)
    {
        if (self::$REMOVE_CHILDREN_FROM_INDEX) {
            $keysCourseData = [];
            foreach ($courseData as $node) {
                $keysCourseData[$node['id_nodo']] = $node;
            }
     ...
     *
     * @param \Lynxlab\ADA\Main\Node\Node $node
     * @return \Lynxlab\ADA\CORE\html4\CBase (either \CText or \CDiv)
     */
