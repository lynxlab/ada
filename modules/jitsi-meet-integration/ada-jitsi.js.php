...
$neededObjAr = [
    ...
    AMA_TYPE_TUTOR => ['layout', 'course', 'course_instance', 'videoroom'],
...
if ($userObj->getType() == AMA_TYPE_STUDENT) {
    $result = $GLOBALS['dh']->courseInstanceTutorInfoGet($courseInstanceObj->getId());
    if (is_array($result)) {
        $result = reset($result);
    }
...
} elseif ($userObj->getType() == AMA_TYPE_TUTOR) {
