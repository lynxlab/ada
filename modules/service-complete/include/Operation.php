...
class Operation
{
    ...
    public function __construct()
    {
        ...
        $this->instanceIDForEval = null;
        $this->userIDForEval = null;
    }
    ...
    public function toArray(&$data = [])
    {
        ...
        if ($this->firstOperand  instanceof self) {
            $operand1 = 'expr(' . $this->firstOperand->toArray($data) . ')';
        } else {
        ...
        if (!is_null($this->operator) && !is_null($this->secondOperand)) {
            ...
            if ($this->secondOperand  instanceof self) {
                $operand2 = 'expr(' . $this->secondOperand->toArray($data) . ')';
            } else {
        ...
        $data[++$key] =  ['id' => $key, 'operator' => $operator, 'operand1' => $operand1, 'operand2' => $operand2, 'priority' => $this->priority];
     ...
     * @access public
     */
    public function evaluate($params = [], &$summary = null)
    {
     ...
     * @access public
     */
    public static function buildOperationTreeFromArray($inputOperations = [])
    {
            ...
            while (!empty($inputOperations) && ($loopCounter-- >= 0)) {
                foreach ($inputOperations as $key => $currentOperation) {
                    ...
                    if (!$op1Pointer && !$op2Pointer) {
                        ...
                        $processed = true;
                    } elseif ($op1Pointer && !$op2Pointer && isset($operationsArray[$op1Pointer])) {
                        // operand 1 is a pointer and its pointed operation has been set up, create the operation
                        $operationsArray[$currentOperation['id']] = self::build(
                            $operationsArray[$op1Pointer],
                            $currentOperation['operator'],
                            $currentOperation['operand2'],
                            $currentOperation['priority']
                        );
                        $processed = true;
                    } elseif (!$op1Pointer && $op2Pointer && isset($operationsArray[$op2Pointer])) {
                        // operand 2 is a pointer and its pointed operation has been set up, create the operation
                        $operationsArray[$currentOperation['id']] = self::build(
                            $currentOperation['operand1'],
                            $currentOperation['operator'],
                            $operationsArray[$op2Pointer],
                            $currentOperation['priority']
                        );
                        $processed = true;
                    } elseif ($op1Pointer && $op2Pointer && isset($operationsArray[$op1Pointer]) && isset($operationsArray[$op2Pointer])) {
     ...
     * @access private
     */
    private static function buildSingleOperationFromPOST($data = [], $operator = ' && ', $priority = 0)
    {
        ...
        for ($i = 0; $i < count($keys); $i++) {
            ...
            if ($i > 0 && is_null($nextKey)) {
                break;
            }

            if ($i == 0) {
                $currentoperand = $data[$currentKey];
            } elseif (isset($operations[$i - 1])) {
            ...
            } elseif (!is_null($currentoperand) && !is_null($nextoperand)) {
     ...
     * @access public
     */
    public static function buildOperationTreeFromPOST($data = [], $opBetweenGroups = null, $opBetweenOperands = null)
    {
        if (is_null($opBetweenGroups)) {
            $opBetweenGroups   = CompleteConditionSet::$opBetweenGroups;
        }
        if (is_null($opBetweenOperands)) {
            $opBetweenOperands = CompleteConditionSet::$opBetweenOperands;
        }
        ...
        foreach ($data as $row) {
            if (!empty($row) && !is_null($row)) {
                $operation = Operation::build(
                    self::buildSingleOperationFromPOST($row, $opBetweenOperands, $priority),
                    $opBetweenGroups,
                    $operation,
                    $priority++
                );
