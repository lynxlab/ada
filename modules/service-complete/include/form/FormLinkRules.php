...
class FormLinkRules
{
     ...
     */
    public function __construct($data = [], $coursesList = [])
    {
        $this->data = $data;
        $this->coursesList = (!is_null($coursesList) && is_array($coursesList)) ? $coursesList : [];
    ...
    private function content()
    {
        ...
        if (isset($this->data['conditionSetId'])) {
            $form->addChild(CDOMElement::create('hidden', 'name:conditionSetId,value:' . $this->data['conditionSetId']));
        }

        if (!empty($this->coursesList)) {
            $labels =  [translateFN('Nome e titolo del corso'), translateFN('collegato alla regola selezionata')];

            foreach ($this->coursesList as $id_course => $courseName) {
                ...
                if (in_array($id_course, $this->data['linkedCourses'])) {
