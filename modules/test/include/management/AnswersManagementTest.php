...
class AnswersManagementTest
{
    ...
    protected function action()
    {
        ...
        $open_cloze = [ADA_NORMAL_TEST_SIMPLICITY,ADA_MEDIUM_TEST_SIMPLICITY];
        if (
            $this->question['tipo'][1] == ADA_OPEN_AUTOMATIC_TEST_TYPE ||
        ...
        $open_answer = false;
        if (in_array($this->question['tipo'][1], [ADA_STANDARD_TEST_TYPE,ADA_MULTIPLE_CHECK_TEST_TYPE])) {
            $open_answer = true;
        }
    ...
    protected function instantiateObject($risposte, $question, $case_sensitive, $open_answer)
    {
        switch ($question['tipo'][1]) {
            ...
            case ADA_CLOZE_TEST_TYPE:
                $modifiable = true;
                $notModifiableSubType = [ADA_ERASE_TEST_SIMPLICITY, ADA_MULTIPLE_TEST_SIMPLICITY];
