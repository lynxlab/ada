...
// Trigger: ClassWithNameSpace. The class AnswerControlTest was declared with namespace Lynxlab\ADA\Module\Test. //
...
class AnswerControlTest extends FormControl
{
    ...
    public function constructComponents()
    {
        if ($this->open_answer) {
            ...
            if ($this->controlData['other_answer']) {
        ...
        $this->controlData['answer'] = Node::prepareInternalLinkMediaForEditor($this->controlData['answer']);

        $input = CDOMElement::create('text', 'name:answer[]');
        $input->setAttribute('class', 'answer');
        $input->setAttribute('value', htmlspecialchars($this->controlData['answer']));
        if ($this->clonable) {
        ...
        $this->item['value']->setAttribute('value', intval($this->controlData['value']));
        ...
        if ($this->show_case_sensitive) {
            ...
            if ($this->controlData['case_sensitive']) {
                $checkbox_value->setAttribute('value', 1);
                $checkbox->setAttribute('checked', '');
            } else {
                $checkbox_value->setAttribute('value', 0);
        ...
        $this->item['record']->setAttribute('value', $this->controlData['record']);
    }
    ...
    public function withData($data)
    {
        ...
        if (is_array($data) && !empty($data)) {
            foreach ($data as $k => $v) {
                if (array_key_exists($k, $this->controlData)) {
