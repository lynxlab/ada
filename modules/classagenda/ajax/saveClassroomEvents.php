...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['instanceID']) && intval($_POST['instanceID']) > 0) {
        ...
        $result = $GLOBALS['dh']->saveClassroomEvents(
            intval($_POST['instanceID']),
            $venueID,
            $postEvents,
            $start,
            $end
        ...
        if (!AMADB::isError($result) && is_array($result)) {
            $retArray = ["status" => "OK", "msg" => translateFN("Calendario salvato")];
            if ($result['newSelectedID'] !== true && intval($result['newSelectedID']) > 0) {
                $retArray['newSelectedID'] = $result;
            }
            if (count($result['generatedIDs'])) {
                $retArray['generatedIDs'] = $result['generatedIDs'];
            }
        } else {
            $retArray = ["status" => "ERROR", "msg" => translateFN("Errore nel salvataggio")];
        }
    } else {
        $retArray = ["status" => "ERROR", "msg" => translateFN("Selezionare un'istanza di corso")];
    }
}
