...
class AMAClassagendaDataHandler extends AMADataHandler
{
    use WithInstance;
    ...
    public function saveClassroomEvents($course_instance_id, $venueID, $eventsArray, $start, $end)
    {
        ...
        if (AMADB::isError($previousEvents)) {
            $previousEvents = [];
        }
        ...
        if (!is_null($eventsArray)) {
            ...
            foreach ($eventsArray as $event) {
                ...
                if (!AMADB::isError($eventID) && intval($eventID) > 0) {
        ...
        return [
            'newSelectedID' => $newSelectedID ?? true,
    ...
    public function getClassRoomEventsForCourseInstance($course_instance_id, $venueID, $start = 0, $end = 0)
    {
        ...
        if (ModuleLoaderHelper::isLoaded('MODULES_CLASSROOM') && !is_null($venueID)) {
        ...
        if (!AMADB::isError($result) && count($result) > 0) {
    ...
    private function saveClassroomEvent($course_instance_id, $eventData)
    {
         ...
         */
        [$date, $time] = explode('T', $eventData['start']);
        [$year, $month, $day] = explode('-', $date);

        $startTimestamp = $this->dateToTs($day . '/' . $month . '/' . $year, $time);
        /**
         * prepare end timestamp
         */
        [$date, $time] = explode('T', $eventData['end']);
        [$year, $month, $day] = explode('-', $date);
    ...
    public function saveRollCallEnterExit($id_student, $id_calendar, $isEntering)
    {
        if ($isEntering) {
            ...
            $params = [$id_student, $id_calendar, $this->dateToTs('now')];
        } else {
            $sql = 'UPDATE `' . self::$PREFIX . 'rollcall` SET `exittime`=? WHERE ' .
                '`id_utente_studente`=? AND `' . self::$PREFIX . 'calendars_id`=? AND ISNULL(`exittime`)';

            $params = [$this->dateToTs('now'), $id_student, $id_calendar];
        }

        return !AMADB::isError($this->queryPrepared($sql, $params));
    }
    ...
    public function getRollCallDetails($id_student, $id_calendar)
    {
        ...
        return $this->getAllPrepared($sql, [$id_student, $id_calendar], AMA_FETCH_ASSOC);
    }
    ...
    public function getRollCallDetailsForInstance($id_student, $id_course_instance)
    {
        ...
        return $this->fixExitTimes(
            $id_course_instance,
            $this->getAllPrepared($sql, [$id_student, $id_course_instance], AMA_FETCH_ASSOC)
        );
    }
    ...
    public function saveReminderForEvent($eventID, $html)
    {
        ...
        if (!MODULES_CLASSAGENDA_EMAIL_REMINDER) {
            ...
            if (!AMADB::isError($evData) && $evData !== false) {
                $isUpdate = true;
    ...
    public function getReminderForEvent($eventID)
    {
        ...
        if (!AMADB::isError($retval) && $retval !== false) {
            ...
            $retval['date'] = Utilities::ts2dFN($retval['creation_date']);
            $retval['time'] = substr(Utilities::ts2tmFN($retval['creation_date']), 0, -3); // remove seconds from time
        }
        return $retval;
    }
    ...
    public function getReminderDataToEmail($reminderID)
    {
        ...
        $result['eventdate'] = Utilities::ts2dFN($result['start']);
        $result['eventstart'] = substr(Utilities::ts2tmFN($result['start']), 0, -3);
        $result['eventend'] = substr(Utilities::ts2tmFN($result['end']), 0, -3);

        if (
            !AMADB::isError($result) && ModuleLoaderHelper::isLoaded('MODULES_CLASSROOM') &&
            ...
            if (!AMADB::isError($classroomresult)) {
                $result['classroomname'] = $classroomresult['name'] ?? '';
                if (isset($classroomresult['id_venue']) && !is_null($classroomresult['id_venue'])) {
                    ...
                    if (!AMADB::isError($venueresult)) {
                        $result['venuename'] = $venueresult['name'] ?? '';
                        $result['venueaddress'] = $venueresult['addressline1'] ?? '';
                        $result['venueaddress'] .= isset($venueresult['addressline2']) ? ' - ' . $venueresult['addressline2'] : '';
                        $result['venuemaplink'] = $venueresult['map_url'] ?? '';
                    }
                }
            }
        }
    ...
    public function getInstanceFullCalendar($instanceID, $withcancelled = false)
    {
        ...
        if (!AMADB::isError($retres)) {
            ...
            foreach ($retres as $i => $result) {
                ...
                $retval[$i]['eventend'] = substr(Utilities::ts2tmFN($result['end']), 0, -3);
                ...
                if (ModuleLoaderHelper::isLoaded('MODULES_CLASSROOM')) {
                    ...
                    if (!is_null($result['id_classroom']) && intval($result['id_classroom']) > 0) {
                        if (!AMADB::isError($classroomresult)) {
                            $retval[$i]['classroomname'] = $classroomresult['name'] ?? '';
                            if (isset($classroomresult['id_venue']) && !is_null($classroomresult['id_venue'])) {
                                $venueresult = $classroomAPI->getVenue($classroomresult['id_venue']);
                                if (!AMADB::isError($venueresult)) {
                                    $retval[$i]['venuename'] = $venueresult['name'] ?? '';
                                    $retval[$i]['venueaddress'] = $venueresult['addressline1'] ?? '';
                                    $retval[$i]['venueaddress'] .= isset($venueresult['addressline2']) ? ' - ' . $venueresult['addressline2'] : '';
                                }
                            }
                        }
                    }
                    if (!isset($retval[$i]['classroomname'])) {
                        $retval[$i]['classroomname'] = '-';
                    }
                    if (!isset($retval[$i]['venuename'])) {
                        $retval[$i]['venuename'] = '-';
                    }
                    if (!isset($retval[$i]['venueaddress'])) {
                        $retval[$i]['venueaddress'] = '-';
                    }
                }
            }
        }
