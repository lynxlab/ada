...
{
    public const INVALID_USERNAME_EXECEPTION_CODE = 49;

    /**
     * class for managing options data
     */
    public const MANAGEMENT_CLASS = __NAMESPACE__ . '\\LdapManagement';
        ...
        if (!is_null($allOptions)) {
            ...
            foreach ($allOptions as $option_id => $options) {
                ...
                if (is_object($loginResult) && $loginResult instanceof ADALoggableUser) {
                    ...
                    return $loginResult;
                } elseif ((is_object($loginResult)) && ($loginResult instanceof Exception)) {
                    if (!in_array($loginResult->getMessage(), $errorMessages)) {
                        $errorMessages[] = $loginResult->getMessage();
                    }
                }
            }
        }
    ...
    public function generateConfigPage()
    {
        ...
        if (!AMADB::isError($optionSetList)) {
            ...
            $OutTable = BaseHtmlLib::tableElement(
                ...
                $tableOutData,
                '',
                translateFN('Elenco delle fonti ' . strtoupper($this->loadProviderName()))
            );
