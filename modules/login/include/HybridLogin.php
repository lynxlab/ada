...
class HybridLogin extends AbstractLogin
{
     ...
     */
    public const MANAGEMENT_CLASS = __NAMESPACE__ . '\\HybridManagement';
        ...
        if (is_object($userObj) && $userObj instanceof ADALoggableUser) {
            if (!is_null($avatar) && !is_null($downloadURL)) {
                ...
                */
                ...
                } elseif (stristr($destFile, 'jpeg') !== false || stristr($destFile, 'jpg') !== false) {
                    imagejpeg($new_img, $destFile);
                } elseif (stristr($destFile, 'gif')) {
    ...
    public function getUserProfile()
    {
        if (is_null($this->authProvider)) {
            $this->authenticate($this->loadProviderName());
        }
    ...
    public function buildADAUserFromProviderObj($user_profile)
    {
        ...
        } elseif (isset($user_profile->email) && strlen($user_profile->email) > 0) {
        ...
        if (strtolower($user_profile->gender) == 'male') {
            $gender = 'M';
        } elseif (strtolower($user_profile->gender) == 'female') {
            $gender = 'F';
        } else {
            $gender = null;
        }
    ...
    public function generateConfigPage()
    {
        ...
        if (!\AMADB::isError($optionSetList)) {
            ...
            $labels =  [translateFN('chiave'), translateFN('valore'),translateFN('azioni')];
            ...
            $OutTable = BaseHtmlLib::tableElement(
                ...
                $tableOutData,
                '',
                translateFN('Opzioni ' . strtoupper($this->loadProviderName()))
            );
