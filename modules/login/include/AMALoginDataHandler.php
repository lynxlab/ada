...
class AMALoginDataHandler extends AMADataHandler
{
    ...
    public static $MODULES_LOGIN_DEFAULT_LOGINPROVIDER = 'adaLogin';
     ...
     * @var string
     */
    public const LDAPCLASSNAME = 'LdapLogin';
    ...
    public function loadOptions($id)
    {
        ...
        $optionsCount = intval(self::$dbToUse->getOnePrepared(
            'SELECT COUNT(DISTINCT(`' . self::$PREFIX . 'providers_options_id`)) FROM `' .
                ...
                self::$PREFIX . 'providers_id` = ? AND `enabled`=1',
            $id
        ));
        ...
        if (!AMADB::isError($res) && is_array($res)) {
            $retArr = [];
            if (count($res) > 0) {
                foreach ($res as $keyvalue) {
                    if (is_numeric($keyvalue['value'])) {
                        ...
                        $value = null;
                    } elseif (strcasecmp($keyvalue['value'], 'true') === 0) {
                        $value = true;
                    } elseif (strcasecmp($keyvalue['value'], 'false') === 0) {
                        $value = false;
                    } elseif (is_string($keyvalue['value']) && is_object(json_decode($keyvalue['value']))) {
                    ...
                    if ($makeArray) {
                        $retArr[$keyvalue[self::$PREFIX . 'providers_options_id']][$keyvalue['key']] = $value;
                    } else {
     ...
     *
     * @return number the provider id|AMAError on Error
     *
     * @access public
     */
    ...
    public function getLoginProvider($provider_id)
    {
        ...
        if (AMADB::isError($res)) {
            ...
            $retval = ['provider_id' => $provider_id];
     ...
     *
     * @return boolean|AMAError|number the created provider id
     *
     * @access public
     */
    public function saveLoginProvider($dataArr)
    {
        ...
        if (!isset($dataArr['provider_id']) || (isset($dataArr['provider_id'])) && intval($dataArr['provider_id']) <= 0) {
            ...
            $res = self::$dbToUse->queryPrepared(
                $sql,
                [$dataArr['className'], $dataArr['name'], $dataArr['buttonLabel'], $orderValue]
            );
            ...
            if (!AMADB::isError($res)) {
                ...
                $optRes = self::$dbToUse->queryPrepared($sql, [$providerID, 1]);
                if (!AMADB::isError($optRes)) {
                    $retval = $providerID;
                } else {
            ...
            $sql = 'UPDATE `' . self::$PREFIX . 'providers` SET' .
                ...
                ' WHERE `' . self::$PREFIX . 'providers_id`=?';
            $retval = self::$dbToUse->queryPrepared(
                $sql,
                [$dataArr['className'], $dataArr['name'], $dataArr['buttonLabel'], $dataArr['provider_id']]
            );
        }
        return $retval;
    }
    ...
    public function deleteLoginProvider($provider_id)
    {
        ...
        // delete them
        if (!AMADB::isError($options)) {
            if (isset($options['optionscount'])) {
                $optionsCount = intval($options['optionscount']);
                unset($options['optionscount']);

                if ($optionsCount > 1) {
                    foreach ($options as $key => $anOption) {
                        $this->deleteOptionSet($key);
                    }
                } elseif (isset($options['providers_options_id'])) {
                    $this->deleteOptionSet($options['providers_options_id']);
                }
            }
        }
        if (AMADB::isError($this->updateOrderBeforeDelete(true, $provider_id))) {
            return new AMAError();
        }
        // delete provider
    ...
    public function setEnabledLoginProvider($provider_id, $status)
    {
        $sql = 'UPDATE `' . self::$PREFIX . 'providers` SET `enabled`=? ' .
            'WHERE `' . self::$PREFIX . 'providers_id`=?';
        return self::$dbToUse->queryPrepared($sql, [intval($status), $provider_id]);
    }
    ...
    public function addLoginToHistory($userID, $time, $loginProviderID, $successfulOptionsID)
    {
        ...
        $rowCount = self::$dbToUse->getOnePrepared($sql, [$userID, $loginProviderID]);

        if (
            !AMADB::isError($rowCount) &&
            $rowCount > self::$MODULES_LOGIN_HISTORY_LIMIT
        ) {
            $numRowsToDel = $rowCount - self::$MODULES_LOGIN_HISTORY_LIMIT + 1;
        ...
        return self::$dbToUse->queryPrepared($sql, [$userID, $time, $loginProviderID, $successfulOptionsID]);
    }
    ...
    public function getAllOptions($provider_id)
    {
        ...
        if (!AMADB::isError($result) && is_array($result) && count($result) > 0) {
            foreach ($result as $anOption) {
                ...
                if (!isset($retval[$option_id]['order'])) {
                    $retval[$option_id]['order'] = $anOption['order'];
                }
                if (!isset($retval[$option_id]['enabled'])) {
                    $retval[$option_id]['enabled'] = $anOption['enabled'];
                }
    ...
    public function deleteOptionSet($options_id)
    {
        ...
        $tablesToDel = ['providers_options', 'options'];
    ...
    public function setEnabledOptionSet($options_id, $status)
    {
        $sql = 'UPDATE `' . self::$PREFIX . 'providers_options` SET `enabled`=? ' .
            'WHERE `' . self::$PREFIX . 'providers_options_id`=?';
        return self::$dbToUse->queryPrepared($sql, [intval($status), $options_id]);
    }
     ...
     *
     * @param int $row_id the row id to be moved
     * @param int $delta amount to move (only +/-1 are tested so far)
     *
     * @return boolean|AMAError
     *
     * @access private
    ...
    private function moveTableRow($row_id, $delta, $isProvider)
    {
        ...
        if (!AMADB::isError($provider_id)) {
            ...
            if (!AMADB::isError($target_id)) {
                ...
                $res = self::$dbToUse->queryPrepared($sql_update_target, [$row_id, $target_id]);
                if (!AMADB::isError($res)) {
    ...
    public function saveOptionByKey($dataArr)
    {
        ...
        if (!is_null($dataArr['newkey']) && $dataArr['key'] != $dataArr['newkey']) {
            ...
            if (!is_null($dataArr['key']) && strlen($dataArr['key']) > 0) {
                ...
                $values = [$dataArr['newkey'], $dataArr['option_id'], $dataArr['key']];
                $retval = $dataArr['newkey'];
            } elseif (is_null($dataArr['key']) || strlen($dataArr['key']) <= 0) {
                $sql = 'INSERT INTO `' . self::$PREFIX . 'options` VALUES (?,?,?)';
                $values = [$dataArr['option_id'], $dataArr['newkey'], null];
                $retval = $dataArr['newkey'];
            }
        } elseif (!is_null($dataArr['key']) && !is_null($dataArr['value'])) {
            $sql = 'UPDATE `' . self::$PREFIX . 'options` SET `value`=? ' .
                'WHERE `' . self::$PREFIX . 'providers_options_id`=? AND `key`=?';
            $values = [$this->orNull($dataArr['value']), $dataArr['option_id'], $dataArr['key']];
    ...
    public function deleteOptionByKey($options_id, $key)
    {
        $sql = 'DELETE FROM `' . self::$PREFIX . 'options` WHERE `' . self::$PREFIX . 'providers_options_id`=?' .
            ' AND `key`=?';
        return self::$dbToUse->queryPrepared($sql, [$options_id, $key]);
    }
    ...
    private function getCommonQueryOptions($isProvider)
    {
        if ($isProvider === false) {
            $options = [
                'table' => self::$PREFIX . 'providers_options',
                'col' => self::$PREFIX . 'providers_options_id',
                'ordercol' => 'order',
            ];
        } else {
            $options = [
                'table' => self::$PREFIX . 'providers',
