...
class CourseViewer
{
     ...
     */
    public static function displayInternalLinkSelector($id_course, $callback_params = [])
    {
        $dh = $GLOBALS['dh'];
    ...
    public static function displayMainIndex($userObj, $id_course, $expand_index, $order, $id_course_instance = null, $container_div_name = null, $dhToUse = null)
    {
        ...
        $callback_params = ['container_div' => $container_div];
    ...
    public static function ordered($course_data, $callback, $callback_params = [], $id_toc = null, $dhToUse = null)
    {
        ...
        if (is_null($id_toc)) {
            ...
            foreach ($course_data as $course_node) {
                if (($r = self::$callback(['node' => $course_node, 'show_hide_span' => false], $callback_params)) != null) {
                    $list_element = CDOMElement::create('li', 'class:courseNode');
                    $list_element->addChild($r);
                    $list->addChild($list_element);
                }
    ...
    public static function struct($course_data, $id_toc, $expand_index, $callback, $callback_params = [], $dhToUse = null)
    {
        ...
        if (!array_key_exists('user_id', $callback_params)) {
            $callback_params['user_id'] = null;
        }
        ...
        $r = self::$callback(['node' => $principale, 'level' => sizeof($s), 'expand_index' => $expand_index, 'show_hide_span' => $show_hide_span], $callback_params);
        ...
        while (!empty($s)) {
            ...
            if (empty($lda[$top_node])) {
                ...
                if (($r = self::$callback(['node' => $nodo, 'level' => $level, 'expand_index' => $expand_index, 'show_hide_span' => $show_hide_span], $callback_params)) != null) {
                    // vito 16 gennaio 2009
                    $li = CDOMElement::create('li', 'class:courseNode');
                    $li->addChild($r);
                    array_push($list, $li);
     ...
     * @return array
     */
    public static function buildLda($result = [])
    {
        $lda = [];
        if (count($result)) {
     ...
     */
    public static function internalLinkSelector($params = [], $external_params = [])
    {
         ...
         */
        if (
            $external_params['action'] == 1 && isset($external_params['id_edited_node'])
     ...
     * @return string $list_item       - an html string for a course index item
     */
    public function nodeSelector($params = [], $external_params = [])
    {
     ...
     * @return string $list_item       - an html string for a course index item
     */
    public static function authorCallback($params = [], $external_params = [])
    {
        $http_root_dir = $GLOBALS['http_root_dir'];
        $show_visits   = !$GLOBALS['hide_visits'];
     ...
     * @return string $list_item       - an html string for a course index item
     */
    public static function tutorCallback($params = [], $external_params = [])
    {
        $http_root_dir = $GLOBALS['http_root_dir'];
        $show_visits   = !$GLOBALS['hide_visits'];
     ...
     * @return string
     */
    public static function studentCallback($params = [], $external_params = [])
    {
        $http_root_dir = $GLOBALS['http_root_dir'];
        $show_visits   = !$GLOBALS['hide_visits'];
     ...
     * @return string $list_item       - an html string for a course index item
     */
    public static function forumStudentCallback($params = [], $external_params = [])
    {
     ...
     * @return string $list_item       - an html string for a course index item
     */
    public static function forumTutorCallback($params = [], $external_params = [])
    {
        return self::forumCommonCallback($params, $external_params);
    }
     ...
     * @return string $list_item       - an html string for a course index item
     */
    public static function forumAuthorCallback($params = [], $external_params = [])
    {
        return self::forumCommonCallback($params, $external_params);
    }

    public static function forumCommonCallback($params = [], $external_params = [])
    {
        $http_root_dir = $GLOBALS['http_root_dir'];

        $show_visits   = !$GLOBALS['hide_visits'];

        $css_classname = self::getClassNameForNote($params['node'], $external_params['user_id'], $external_params['class_tutors_ids'] ?? null);
        ...
        if (isset($params['node']['id_utente'])) {
            ...
            } elseif (is_readable(ADA_UPLOAD_PATH . DIRECTORY_SEPARATOR . $params['node']['id_utente'] . DIRECTORY_SEPARATOR . $params['node']['avatar'])) {
            ...
            } elseif (is_readable(ADA_UPLOAD_PATH . ADA_DEFAULT_AVATAR)) {
     ...
     * @return string
     */
    public function forumGetNoteIcon($node, $class_tutor_id)
    {
     ...
     * @return string $list_item       - an html string for a course index item
     */
    public function getClassNameForForumUserName($node, $id_user)
    {
     ...
     * @param  array  $params          - an array of parameters
     * @param  array  $external_params - an array with additional parameters
     * @return \Lynxlab\ADA\CORE\html4\CBase $list_item       - an html string for a course index item
     */
    public static function getDisclosureElement($params = [], $external_params = [])
    {
        // vito 13 gennaio 2009
        ...
        if (
            ($params['node']['tipo'] == ADA_GROUP_TYPE && $hide_span)
