...
function copyNodeFN($id_node, $id_course, $action)
{
    ...
    if (is_object($nodeObj) && (!AMADataHandler::isError($nodeObj))) {
        ...
        $menu = "<a href=$http_root_dir/browsing/view.php?id_node=$sess_id_node>" . translateFN('Torna') . "</a>";
        $head_form .=  "<b>" . translateFN('ID del nodo') . "</b> $id_node<br>\n";
...
function getNodeDataFromPost($post_data = [])
{
    $node_data = [
    'id' => $post_data['id'],
    'name' => $post_data['name'],
    ...
    'n_contacts' => $post_data['n_contacts'] ?? null,
    'icon' => $post_data['icon'],
    'id_node_author' => $post_data['id_node_author'],
    'creation_date' => $post_data['creation_date'],
    'DataFCKeditor' => $post_data['DataFCKeditor'],
    'DataFCK_hyphen' => $post_data['DataFCK_hyphen'] ?? null,
    'hyphenation' =>  $post_data['ADACodeHyphen'] ?? null,
    'DataFCK_grammar' => $post_data['DataFCK_grammar'] ?? null,
    'grammar' => $post_data['ADACodeGrammar'] ?? null,
    'DataFCK_semantic' => $post_data['DataFCK_semantic'] ?? null,
    'semantic' => $post_data['ADACodeSemantic'] ?? null,
    'DataFCK_notes' => $post_data['DataFCK_notes'] ?? null,
    'notes' => $post_data['ADACodeNotes'] ?? null,
    'DataFCK_examples' => $post_data['DataFCK_examples'] ?? null,
    'examples' => $post_data['ADACodeExamples'] ?? null,
    'DataFCK_exlanguage' => $post_data['DataFCK_exlanguage'] ?? null,
    'exlanguage' => $post_data['ADACodeExlanguage'] ?? null,
    // @author giorgio 26/apr/2013
    'forcecreationupdate' => $post_data['forcecreationupdate'] ?? null,
    // @author giorgio 14/jun/2019
    'is_forkedpaths' => $post_data['is_forkedpaths'] ?? null,
    ];
