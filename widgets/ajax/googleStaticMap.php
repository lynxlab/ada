...
require_once Widget::ADA_WIDGET_AJAX_ROOTDIR . '/include/GoogleAccess.inc.php';

if (!isset($url)) {
    die('No url  to be loaded.');
}
...
if (isset($settings['staticmapAPI']) && isset($settings['staticmapAPI']['apiKEY'])) {
    ...
    if (isset($settings['staticmapAPI']['privateKEY']) && strlen($settings['staticmapAPI']['privateKEY']) > 0) {
        ...
        $decodedKey = base64_decode(str_replace(['-', '_'], ['+', '/'], $settings['staticmapAPI']['privateKEY']));
    ...
    die('Non static map credentials found.');
}
