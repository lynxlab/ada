...
{
    // MARK: Chat
    public static function getChat($data = '', ADALoggableUser $userObj = null, $event_token = null)
    {
        if (is_null($userObj)) {
            $userObj = new ADAGuest();
        }
        ...
        if (!is_null($event_token) && $userObj instanceof ADAPractitioner) {
     ...
     */
    public static function getEventProposalForm($id_user, $data = [], $errors = [], $tester = null)
    {
    ...
    public static function getProposedEventForm($data = [], $errors = [], $tester = null)
    {
        ...
        } elseif ($data['flags'] & ADA_VIDEOCHAT_EVENT) {
            $type->addChild(new CText('Appuntamento in videochat'));
        } elseif ($data['flags'] & ADA_PHONE_EVENT) {
            $type->addChild(new CText('Appuntamento telefonico'));
        } elseif ($data['flags'] & ADA_IN_PLACE_EVENT) {
     ...
     * @return unknown_type
     */
    public static function getConfirmedEventProposalForm($data = [])
    {
    ...
    public static function getEventsAsTable(ADAGenericUser $userObj, $data_Ar = [], $testers_data_Ar = [])
    {
        if (empty($data_Ar)) {
            return new CText('');
        }
        ...
        */
        ...
        foreach ($data_Ar as $tester => $appointment_data_Ar) {
            ...
            foreach ($appointment_data_Ar as $appointment_id => $appointment_Ar) {
                ...
                $appointments_Ar[] = [$data_msg,$subject_link,$sender_username,$priority];
            }
        }
    ...
    public static function getMessagesAsTable($data_Ar = [], $testers_dataAr = [])
    {
        if (empty($data_Ar)) {
            return new CText(translateFN('Non sono presenti messaggi'));
        }
        return self::displayMessagesAsTable($data_Ar, ADA_MSG_SIMPLE, $testers_dataAr);
    }


    private static function getMessagesFormHeader($header_dataAr = [])
    {
        $thead_dataAr = [];
    ...
    private static function getSentMessagesFormContent($data_Ar = [], $testers_dataAr = [])
    {
        ...
        foreach ($data_Ar as $tester => $message_dataAr) {
            ...
            foreach ($message_dataAr as $message_Ar) {
                ...
                $messages_Ar[] = [$addressee_username, $data_msg, $subject_link];//, $delete, $action_link);
            }
        }
        return $messages_Ar;
    }
    ...
    private static function getReceivedMessagesFormContent($data_Ar = [], $testers_dataAr = [])
    {
        ...
        foreach ($data_Ar as $tester => $message_dataAr) {
            ...
            foreach ($message_dataAr as $message_id => $message_Ar) {
                ...
                if (isset($sender_name_surname)) {
                    $messages_Ar[] = [$sender_name_surname, $data_msg, $subject_link, $priority, $delete, $read, $action_link];
                } else {
                    $messages_Ar[] = [$sender_username, $data_msg, $subject_link, $priority, $delete, $read, $action_link];
                }
            }
        }
        return $messages_Ar;
    ...
    public static function getSentMessagesAsForm($data_Ar = [], $testers_dataAr = [])
    {
        if (empty($data_Ar)) {
            return new CText(translateFN('Non sono presenti messaggi'));
        }
        ...
        $header_dataAr = [
        ['text' => 'Destinatario'],
        ['text' => 'Data'],// 'action' => 'list_messages.php?sort_field=data_ora'),
        ['text' => 'Oggetto'],// 'action'=> 'list_messages.php?sort_field=titolo'),
        // array('text' => 'Cancella'),
        // array('text' => '')
        ];
    ...
    public static function getReceivedMessagesAsForm($data_Ar = [], $testers_dataAr = [])
    {
        if (empty($data_Ar)) {
            return new CText(translateFN('Non sono presenti messaggi'));
        }
        ...
        $header_dataAr = [
        ['text' => 'Mittente'],// 'action' => 'list_messages.php?sort_field=id_mittente'),
        ['text' => 'Data'],// 'action' => 'list_messages.php?sort_field=data_ora'),
        ['text' => 'Oggetto'],// 'action'=> 'list_messages.php?sort_field=titolo'),
        ['text' => 'Priorit&agrave;'],
        ['text' => 'Cancella'],
    ...
    public static function getAgendaAsTable($data_Ar = [], $testers_dataAr = [])
    {
        if (empty($data_Ar)) {
            return new CText(translateFN('Non sono presenti appuntamenti'));
        }
        return self::displayMessagesAsTable($data_Ar, ADA_MSG_AGENDA, $testers_dataAr);
    }

    public static function getAgendaAsForm($data_Ar = [], $testers_dataAr = [])
    {
        if (empty($data_Ar)) {
            return new CText(translateFN('Non sono presenti appuntamenti'));
        }
        return self::displayMessagesAsForm($data_Ar, ADA_MSG_AGENDA, $testers_dataAr);
    }
    ...
    private static function displayMessagesAsForm($data_Ar = [], $message_type = ADA_MSG_SIMPLE, $testers_dataAr = [])
    {
        ...
        foreach ($data_Ar as $tester => $appointment_data_Ar) {
            ...
            foreach ($appointment_data_Ar as $appointment_id => $appointment_Ar) {
                ...
                $appointments_Ar[] = [$sender_username, $data_msg, $subject_link, $priority, $delete, $read, $action_link];
            }
        }
    ...
    public static function getOperationWasSuccessfullView($text)
    {
    ...
    private static function getEventProposalFormHoursSelect($select_id, $preselect_time_value = null)
    {
        $start_hour = 8;
        $end_hour   = 24;
