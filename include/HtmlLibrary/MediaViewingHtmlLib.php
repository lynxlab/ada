...
class MediaViewingHtmlLib
{
    ...
    private static function jplayerCommonJS($format, $divID, $url, $title = null, $width = null, $height = null)
    {
         ...
         */
        if ($format == 'never') {
            $jplayerCode .= 'poster: "' . HTTP_ROOT_DIR . '/layout/"+ADA_TEMPLATE_FAMILY+"/img/header-logo.png",';
        }
        $jplayerCode .= $format . ': "' . $url . '"
									})';
        if (defined('JPLAYER_AUTOPLAY') && JPLAYER_AUTOPLAY) {
            $jplayerCode .= '.jPlayer("play")';
        }
        $jplayerCode .= ';},
								play: function() {
        ...
        if ($format == 'm4v') {
            $jplayerCode .= 'solution: "html, flash",';
        }
