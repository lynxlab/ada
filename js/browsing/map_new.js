...
function transform( line, deg){
	...
	else line.style.filter =  "progid:DXImageTransform.Microsoft.Matrix(M11="+Math.cos(deg)+", M12="+(- Math.sin(deg))+", M21="+ Math.sin(deg)+", M22="+(Math.cos(deg))+",sizingMethod='auto expand')";
...
function setMapMinY(){
	...
	for( var i = 0; i < $j(".newNodeMap").length; i++){
		...
		if( parseInt(thisNode.style.top.replace("px","")) > maxY ) maxY = parseInt(thisNode.style.top.replace("px",""))
...
var Node = function( element ){
	...
	new Draggable(element, {
		...
		onDrag: function(drgObj){
			...
			var parent = drgObj.element.parentNode
...
function setChildDisposition(){
	...
	for(i = 0; i < this.nodeList.length; i++){
		...
		for( j = 0; j < this.nodeList.length; j++){
			...
			if( elem != elemToContr && elem.isInRect(elemToContr.rect) && !!elem.orig){
...
var Map = function(){
	...
	for( var i = 0; i < $j(".newNodeMap").length; i++){
		...
		var X1 = X0 + parseInt( thisNode.style.width.replace("px","").replace("pt","") );
...
function newLine(element, node, X0, Y0, X1, Y1){
	...
	if( !isIE ){// line rotate around his centre
	...
	}else{// line rotate 'attached' to his top and left side ( need four cases )
